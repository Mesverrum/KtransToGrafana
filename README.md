...
**You are reading the `multicontainer_netbox` branch.**
...
Add cron entries on the host so new devices added to NetBox get picked up automatically. Stagger each group a few minutes apart so they don't all query NetBox at once:
```
...
```
Each run queries NetBox with the group's filter, atomically publishes a fresh `state/devices-<group>.yaml`, and sends `SIGUSR2` to the matching poller so it picks up the new device list without a restart. (`SIGUSR2` is ktranslate's reload signal — `SIGHUP` has no handler and would terminate the container.) If discovery returns zero devices (NetBox unreachable, filter matched nothing) the script preserves the previous device list rather than wiping it. If the device list is unchanged from the previous run, no reload is sent.
...
```
You should get one row per polled device, grouped by which credential group is polling it. If the table is empty after a couple minutes: check `make logs` for ktranslate activity, confirm `make discover GROUP=<name>` populated `state/devices-<name>.yaml`, and confirm `snmpwalk` works from the Docker host to one of the devices NetBox returned (see `troubleshooting/snmp.md`).
