...
```
Each run queries NetBox with the group's filter, atomically publishes a fresh `state/devices-<group>.yaml`, and sends a SIGHUP to the matching poller so it picks up the new device list without a restart. If discovery returns zero devices (NetBox unreachable, filter matched nothing) the script preserves the previous device list rather than wiping it. If the device list is unchanged from the previous run, no reload is sent.
