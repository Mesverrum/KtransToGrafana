...
# Then bring the stack up with: make up
...
if [[ ${#GROUP_FILES[@]} -eq 0 ]]; then
  ...
  echo "       Copy groups/onboarding.env.sample to groups/onboarding.env to define a group." >&2
  exit 1
fi
...
# Merge at runtime via: make up  (bash scripts/compose-files.sh)
services:
EOF
...
{
  echo "# GENERATED by scripts/generate-groups.sh — do not edit by hand."
  echo "# Merge at runtime via: make up"
  echo "services:"
